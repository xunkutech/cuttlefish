...
//    private static final Logger log = Logger.getLogger(PrivateKeyReader.class.getName());
    ...
    private static Map<String, PrivateKey> keyCache = new ConcurrentHashMap<>();
     ...
     * @throws IOException
     */
    public static PrivateKey getPrivateKey(String fileName) throws IOException, InvalidException {
        ...
        key = getPrivateKey(bos.toByteArray());
        is.close();
     ...
     * @throws IOException
     */
    public static PrivateKey getPrivateKey(byte[] inKey) throws InvalidException {

        ByteArrayInputStream bis = new ByteArrayInputStream(inKey);
        ...
        try {
            ...
            throw new InvalidException("JCE error: " + e.getMessage()); //$NON-NLS-1$
        }
        try {
            while ((line = in.readLine()) != null) {
                if (line.indexOf(P1_BEGIN_MARKER) != -1) {
                    byte[] keyBytes = readKeyMaterial(P1_END_MARKER, in);
                    KeySpec keySpec = getRSAKeySpec(keyBytes);

                    try {
                        return factory.generatePrivate(keySpec);
                    } catch (InvalidKeySpecException e) {
                        throw new InvalidException("Invalid PKCS#1 PEM file: " + e.getMessage()); //$NON-NLS-1$
                    }
                }

                if (line.indexOf(P8_BEGIN_MARKER) != -1) {
                    byte[] keyBytes = readKeyMaterial(P8_END_MARKER, in);
                    KeySpec keySpec = new PKCS8EncodedKeySpec(keyBytes);

                    try {
                        return factory.generatePrivate(keySpec);
                    } catch (InvalidKeySpecException e) {
                        throw new InvalidException("Invalid PKCS#8 PEM file: " + e.getMessage()); //$NON-NLS-1$
                    }
                }
        ...
        throw new InvalidException("Invalid PEM file: no begin marker"); //$NON-NLS-1$
    }
     ...
     * @throws IOException
     */
    private static byte[] readKeyMaterial(String endMarker, BufferedReader in) throws InvalidException {
        String line = null;
        StringBuffer buf = new StringBuffer();
        try {
            while ((line = in.readLine()) != null) {
                if (line.indexOf(endMarker) != -1) {
                    return CodecUtils.fromBase64(buf.toString());
                }
        ...
        throw new InvalidException("Invalid PEM file: No end marker"); //$NON-NLS-1$
    }
    ...
    private static RSAPrivateCrtKeySpec getRSAKeySpec(byte[] keyBytes) throws InvalidException {

        DerParser parser = new DerParser(keyBytes);

        Asn1Object sequence = parser.read();
        if (sequence.getType() != DerParser.SEQUENCE)
            throw new InvalidException("Invalid DER: not a sequence"); //$NON-NLS-1$
...
class DerParser {
     ...
     * @throws IOException
     */
    public Asn1Object read() throws InvalidException {
        try {
            int tag = in.read();

            if (tag == -1)
                throw new InvalidException("Invalid DER: stream too short, missing tag"); //$NON-NLS-1$
            ...
            int n = in.read(value);
            if (n < length)
                throw new InvalidException("Invalid DER: stream too short, missing value"); //$NON-NLS-1$
...
class Asn1Object {
     ...
     * @throws IOException
     */
    public DerParser getParser() throws InvalidException {
        if (!isConstructed())
            throw new InvalidException("Invalid DER: can't parse primitive entity"); //$NON-NLS-1$
     ...
     * @throws IOException
     */
    public BigInteger getInteger() throws InvalidException {
        if (type != DerParser.INTEGER)
            throw new InvalidException("Invalid DER: object is not integer"); //$NON-NLS-1$
     ...
     * @throws IOException
     */
    public String getString() throws InvalidException {
        ...
        switch (type) {
            ...
            case DerParser.UNIVERSAL_STRING:
                throw new InvalidException("Invalid DER: can't handle UCS-4 string"); //$NON-NLS-1$

            default:
                throw new InvalidException("Invalid DER: object is not a string"); //$NON-NLS-1$
        }
