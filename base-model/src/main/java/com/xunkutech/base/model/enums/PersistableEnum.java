...
//                ConvertClass convertClass = this.getClass().getDeclaredAnnotation(ConvertClass.class);
//
//                if (null == convertClass) {
//                    throw new IllegalStateException("The convert is not apply the ConvertClass annotation:" + this.getClass().getCanonicalName());
//                }
//                @SuppressWarnings("unchecked")
//                Class<ENUM> enumClass = (Class<ENUM>) convertClass.value();
                List<ENUM> findResult = Arrays.stream(enumClass.getEnumConstants())
                        .filter(enumInstance -> dbData.equals(enumInstance.getEnumKey()))
                        .collect(toList());
                if (findResult.size() != 1) {
                    if (findResult.size() > 1) {
                        throw new IllegalArgumentException("Persistent enum has duplicated value: " + dbData.toString() + ", for " + findResult.size() + " times.");
                    }
                    throw new IllegalArgumentException("Persistent enum has not defined this value: " + dbData.toString());
                }
